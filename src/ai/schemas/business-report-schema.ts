...
export const AnalyzeBusinessCreditReportOutputSchema = z.object({
  unlockScore: z.number().min(0).max(1000).describe("The AI-driven Unlock Score™ from 0 to 1000, representing how ready the business is for funding."),
  ...
  unlockTier: z.string().describe("The Unlock Score™ tier label (e.g., High Risk, Fundable, Highly Fundable, Unlock VIP)."),
  businessSummary: z.object({
      ...
      summaryText: z.string().describe("A professional summary of the business's current funding readiness, entity details, and online presence."),
  }),
  ...
  riskFactors: z.array(z.string()).describe("A list of identified red flags or risks holding back the business's Unlock Score (e.g., 'Website not found', 'No Google reviews', 'UCC filings present')."),
  actionPlan: z.array(z.string()).describe("A list of 3-5 specific, actionable steps to improve the business's Unlock Score™ and become bank-ready."),
